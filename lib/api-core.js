...
async function runBatchQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs = 3000 }) {
  ...
  const json = extractJson(result.text);
  const answerText = chatContent(json) || result.text;
  ...
  if (!answers) {
    ...
    const error = new Error("Single prompt QA response was not parseable.");
    error.code = "SINGLE_PROMPT_PARSE_ERROR";
    throw error;
  }
...
async function runQuickQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs = 20000 }) {
  const startedAt = Date.now();
  try {
    return await runBatchQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs });
  } catch (error) {
    if (error.code !== "SINGLE_PROMPT_PARSE_ERROR") throw error;

    const batchElapsedMs = Date.now() - startedAt;
    const fallbackBundle = await runQaCases({
      chatUrl,
      apiKey,
      model,
      qaCases,
      tryBatch: false,
      qaCaseTimeoutMs: timeoutMs,
    ...
    return {
      ...
      timings: {
        batchElapsedMs,
        fallbackElapsedMs: fallbackBundle.timings.totalElapsedMs,
        totalElapsedMs,
      },
    };
  }
