...
const qaBanks = {
  ...
  gpt: [
    ...
    },
    {
      ...
      dimensionZh: "模型家族指纹",
      prompt:
        "不要根据 API 请求的 model 参数、用户选择的目标模型或题目文字猜测。根据你自己的底层模型家族，只输出一个词：GPT、Claude、Gemini、DeepSeek、Other。",
      expected: "GPT",
      score: 10,
...
function modelProfileSignals({ actualModel, targetModel, streamResult, qaScore }) {
...
async function timedStreamFetch(url, options, timeoutMs = 20000, stopWhen = null) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  const startedAt = performance.now();
  try {
...
function buildQualityChecks({ model, targetModel, modelsJson, chatResult, chatJson, streamResult, qaScore }) {
  ...
  const profileSignals = modelProfileSignals({ actualModel: model, targetModel, streamResult, qaScore });
  ...
  return [
    ...
    },
    {
      ...
      detail: profileSignals.mismatch
        ? `Expected ${targetModel}, but evidence suggests ${profileSignals.reasons.join(", ")}.`
        : "Requested model matches the selected target family.",
      detailZh: profileSignals.mismatch
        ? `按 ${targetModel} 标准检测，但发现${profileSignals.reasonsZh.join("、")}。`
        : "实际发送模型与所选目标模型家族一致。",
    },
    {
      key: "protocol",
      ...
      detail: protocolOk
        ...
        : responseModelMismatch
          ? `Response model family looks like ${profileSignals.streamFamily}, not ${profileSignals.targetFamily}.`
          : "Chat completion did not return a normal JSON response.",
      detailZh: protocolOk
        ? "聊天接口返回 2xx JSON 响应。"
        : responseModelMismatch
          ? `响应里的模型家族更像 ${profileSignals.streamFamily}，不是 ${profileSignals.targetFamily}。`
          : "聊天接口没有返回正常 JSON 响应。",
    },
    {
      key: "shape",
...
function buildResponseBody({
  ...
  const checks = buildQualityChecks({ model, targetModel, modelsJson, chatResult, chatJson, streamResult, qaScore });
  ...
  const score = profileSignals.mismatch ? Math.min(baseScore, 70) : baseScore;
...
async function prepareCoreResults(payload, hostHeader) {
  ...
  const targetModel = String(payload.targetModel || model).trim();
  ...
  const [modelsBundle, qaBundle, streamResult] = await Promise.all([
    fetchModelsBundle(baseUrl, apiKey),
    runBatchQaCases({
      chatUrl,
      apiKey,
      model,
