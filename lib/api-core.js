...
async function qaResultForCase({ chatUrl, apiKey, model, qaCase, timeoutMs = 12000 }) {
  ...
  try {
    ...
    let source = "chat";
    let rawPreview = truncateText(result.text);
    let latencyMs = result.timings.firstByteMs;
    let status = result.status;
    if (result.ok && !answer) {
      try {
        const streamFallback = await fallbackToStream();
        if (streamFallback.ok) {
          answer = streamFallback.answer;
          source = "stream-fallback";
          rawPreview = streamFallback.rawPreview;
          latencyMs = streamFallback.latencyMs;
          status = streamFallback.status;
        }
      } catch {
        // Keep the original chat result when the fallback request also fails.
      }
    }
    return {
      id: qaCase.id,
    ...
    try {
      const streamFallback = await fallbackToStream();
      if (streamFallback.ok) {
        const answer = streamFallback.answer;
        return {
          id: qaCase.id,
          dimension: qaCase.dimension,
          dimensionZh: qaCase.dimensionZh,
      ...
      // The error response below reports the original chat failure.
    }
...
async function runQaCases({ chatUrl, apiKey, model, qaCases }) {
  ...
  for (const qaCase of qaCases) {
    results.push(await qaResultForCase({ chatUrl, apiKey, model, qaCase }));
  }
  return results;
}
  ...
  return {
    ...
    body: {
      ...
      chat: {
        ...
        usage: usageTokens(chatJson),
        outputText: chatContent(chatJson),
      },
    },
  };
}
