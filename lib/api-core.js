...
function batchQaAnswers(answerText, qaCases) {
  ...
  for (const parser of parsers) {
    ...
    if (!answers) continue;
    const hasAllAnswers = qaCases.every((qaCase) => answers[qaCase.id] !== undefined);
    if (!hasAllAnswers) continue;
    const allPass = qaCases.every((qaCase) => qaCase.judge(normalizeBatchAnswerValue(answers[qaCase.id])));
    if (allPass) return answers;
  }
...
async function qaResultForCase({ chatUrl, apiKey, model, qaCase, timeoutMs = 8000 }) {
...
async function runBatchQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs = 6500 }) {
...
async function runQaCases({ chatUrl, apiKey, model, qaCases }) {
  ...
  try {
    const batchBundle = await runBatchQaCases({ chatUrl, apiKey, model, qaCases });
  ...
  return {
    ...
    batchError: batchError?.message || "Batch QA failed.",
    timings: {
