...
// `new URL("/v1/models", baseUrl)` treats a leading slash as an absolute-path
// reference, which discards baseUrl's own path (e.g. a proxy/gateway prefix
// like "/webhook/router") and resolves against the origin only. That silently
// sends requests to the wrong place for any baseUrl that isn't a bare origin
// or "/v1". Plain string concatenation keeps whatever path the user provided.
function joinUrl(baseUrl, endpointPath) {
  // Callers always pass an endpointPath starting with "v1/...", so if baseUrl
  // already ends in "/v1" (a common convention for OpenAI-SDK-style base
  // URLs), drop that trailing segment first to avoid "/v1/v1/...".
...
function modelAvailabilitySignal({ model, modelsResult, modelsJson, chatResult, streamResult }) {
  const models = Array.isArray(modelsJson?.data) ? modelsJson.data : [];
  const modelIds = models.map((item) => item?.id).filter(Boolean);
  const modelListed = modelIds.includes(model);
  const directAccepted = Boolean(chatResult?.ok || streamResult?.ok);
  ...
  if (directAccepted) {
    return {
      status: modelsResult?.skipped ? "pass" : "partial",
      score: modelsResult?.skipped ? 10 : 5,
      detail: modelsResult?.skipped
        ? "Direct request accepted this model id. /v1/models was not rechecked for this fixed target."
        : "Selected model was not found in /v1/models, but a direct request accepted it.",
      detailZh: modelsResult?.skipped
        ? "直接请求接受了这个模型 ID。本次固定模型检测没有重新校验 /v1/models。"
        : "所选模型没有出现在 /v1/models 中，但直接请求可以使用。",
    };
  }
  ...
  return {
    ...
    score: 0,
    detail: modelsResult?.skipped
      ? "Direct requests for this model failed. /v1/models was not rechecked for this fixed target."
      : "Selected model was not found in /v1/models.",
    detailZh: modelsResult?.skipped
      ? "这个模型的直接请求失败。本次固定模型检测没有重新校验 /v1/models。"
...
function buildQualityChecks({ model, targetModel, modelsResult, modelsJson, chatResult, chatJson, streamResult, qaScore, selectionMode = "fixed" }) {
  const modelAvailability = modelAvailabilitySignal({ model, modelsResult, modelsJson, chatResult, streamResult });
  ...
  const responseStructureOk = Boolean(chatResult.ok && responseOk);
  ...
  return [
    ...
    },
    {
      ...
      detail: modelAvailability.detail,
      detailZh: modelAvailability.detailZh,
    },
    {
      key: "profile",
      label: "Model profile",
    ...
    },
    {
      ...
      labelZh: "响应结构",
      status: statusLabel(responseStructureOk),
      detail: responseStructureOk
        ? "Response includes an OpenAI-style choices array."
        : chatResult.ok
          ? "Response is missing the expected choices array."
          : "Chat request failed, so response structure could not be verified.",
      detailZh: responseStructureOk
        ? "响应包含 OpenAI 风格的 choices 数组。"
        : chatResult.ok
          ? "响应缺少预期的 choices 数组。"
          : "聊天请求失败，因此无法验证响应结构。",
    },
...
function latencyScore(latencyMs, qaScore) {
  ...
  let score = 0;
  if (chat <= 2500 && ttft <= 1500) score = 10;
  else if (chat <= 6000 && ttft <= 3000) score = 7;
  else if (chat <= 10000 && ttft <= 6000) score = 4;
