...
const qaCases = [
  {
    ...
    answerPattern: /\bparis\b/i,
  },
  {
    prompt: "Answer with digits only: What is 17 + 28?",
    answerPattern: /\b45\b/,
  },
  {
    prompt: "Answer with exactly the requested word and nothing else: reply PURPLE",
    answerPattern: /^\s*purple\s*$/i,
  },
];
  ...
  return [
    {
      ...
      detail: `Answered ${qaScore.passed}/${qaScore.total} control questions correctly.`,
      detailZh: `${qaScore.total} 道客观题答对 ${qaScore.passed} 道。`,
    },
    {
      key: "model",
...
export async function buildTestResult(payload, hostHeader) {
  ...
  const chatPrompt = payload.prompt || qaCases[0].prompt;
  ...
  for (const qaCase of qaCases) {
    ...
    qaResults.push({
      ...
      answer,
      ok: result.ok && qaCase.answerPattern.test(answer),
