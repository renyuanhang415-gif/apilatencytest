  ...
  return [
    {
      ...
      detail: `Answered ${qaScore.passed}/${qaScore.total} control questions correctly. Matched ${qaScore.bankLabel} question bank.`,
      detailZh: `通过 ${qaScore.passed}/${qaScore.total} 道控制题，已匹配 ${qaScore.bankLabel} 题库。`,
    },
    {
      key: "model",
...
export async function buildTestResult(payload, hostHeader) {
  ...
  const qaResults = [];
  for (const qaCase of qaBank.cases) {
    try {
