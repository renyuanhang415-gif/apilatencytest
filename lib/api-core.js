...
function batchQaAnswers(answerText) {
  const json = extractJson(stripMarkdownFence(answerText));
...
async function runBatchQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs = 12000 }) {
  ...
  try {
    ...
    answerText = chatContent(extractJson(result.text));
    if (!answerText) throw new Error("Empty batch chat answer");
  } catch {
    source = "batch-stream";
    result = await timedStreamFetch(
      chatUrl,
      {
        method: "POST",
        headers: {
...
async function prepareCoreResults(payload, hostHeader) {
  ...
  const chatPrompt = "Answer with exactly one word: OK";
  ...
  const chatJson = extractJson(chatResult.text);
  const qaResults = await runQaCases({ chatUrl, apiKey, model, qaCases: qaBank.cases });
  return {
    status: 200,
    baseUrl,
    apiKey,
