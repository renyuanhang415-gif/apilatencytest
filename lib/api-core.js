...
function modelProfileMismatch(actualModel, targetModel) {
  if (!targetModel || actualModel === targetModel) return false;
  const actualFamily = modelFamilyForId(actualModel);
  const targetFamily = modelFamilyForId(targetModel);
  if (actualFamily === "generic" || targetFamily === "generic") return false;
  return actualFamily !== targetFamily;
}
...
function buildQualityChecks({ model, targetModel, modelsJson, chatResult, chatJson, streamResult, qaScore }) {
  ...
  const profileMismatch = modelProfileMismatch(model, targetModel);
  ...
  return [
    ...
    },
    {
      ...
      label: "Model profile",
      labelZh: "模型特征校验",
      status: statusLabel(!profileMismatch),
      detail: profileMismatch
        ? `Expected ${targetModel}, but request was sent to ${model}.`
        : "Requested model matches the selected target family.",
      detailZh: profileMismatch
        ? `按 ${targetModel} 标准检测，但实际发送的模型 ID 为 ${model}。`
        : "实际发送模型与所选目标模型家族一致。",
    },
    {
      key: "protocol",
...
function buildResponseBody({
  ...
  const score = profileMismatch ? Math.min(baseScore, 70) : baseScore;
