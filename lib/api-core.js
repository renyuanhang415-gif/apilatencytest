...
function modelProfileSignals({ actualModel, targetModel, streamResult, qaScore }) {
  ...
  return {
    ...
    strongMismatch: strongReasons.length > 0,
    reasons,
    reasonsZh,
    strongReasons,
    targetFamily,
...
function buildResponseBody({
  ...
  const score = profileSignals.strongMismatch ? Math.min(baseScore, 70) : profileSignals.mismatch ? Math.min(baseScore, 90) : baseScore;
