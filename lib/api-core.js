...
function batchQaPrompt(qaCases) {
  ...
  return [
    ...
    "Use the same IDs as keys.",
    'Example: {"C1":"PURPLE","C2":"45"}',
    ...lines,
  ].join("\n");
}

function batchQaAnswers(answerText) {
...
function classifyEvidence(ok, pending = false, fallback = "fail") {
  if (pending) return "inconclusive";
  if (ok === true) return "pass";
  if (ok === false) return fallback;
  return "inconclusive";
}

function evidenceStatusDetail(status, passText, failText, inconclusiveText) {
  if (status === "pass") return passText;
  if (status === "fail" || status === "warning") return failText;
  return inconclusiveText;
}

function usageConsistencySummary(streamSummary) {
  const inputSamples = streamSummary.messageDeltaInputTokensSamples || [];
  const outputSamples = streamSummary.outputTokensSamples || [];
  const baselineInput = streamSummary.messageStartInputTokens;
  const inputConsistent =
    baselineInput === null ||
    inputSamples.length === 0 ||
    inputSamples.every((value) => value === baselineInput);
  const outputMonotonic =
    outputSamples.length <= 1 ||
    outputSamples.every((value, index) => index === 0 || value >= outputSamples[index - 1]);
  return {
    hasUsage: baselineInput !== null || outputSamples.length > 0,
    inputConsistent,
    outputMonotonic,
    consistent: inputConsistent && outputMonotonic,
  };
}

function responseShapeSummary(chatJson, selectedModel) {
  const choice = chatJson?.choices?.[0];
  const usage = usageTokens(chatJson);
  const responseModel = chatJson?.model || null;
  const hasChoicesArray = Array.isArray(chatJson?.choices) && chatJson.choices.length > 0;
  const hasMessageContent = typeof choice?.message?.content === "string" || Array.isArray(choice?.message?.content);
  const hasDeltaContent = typeof choice?.delta?.content === "string" || Array.isArray(choice?.delta?.content);
  const hasUsage = usage.input !== null || usage.output !== null;
  const idLooksNormal = typeof chatJson?.id === "string" && chatJson.id.length >= 6;
  return {
    responseModel,
    modelMatch: Boolean(responseModel && selectedModel && responseModel === selectedModel),
    hasChoicesArray,
    hasMessageContent,
    hasDeltaContent,
    hasUsage,
    idLooksNormal,
    looksCompatible: hasChoicesArray && (hasMessageContent || hasDeltaContent),
  };
}

function detectionEvidence({ model, chatJson, streamResult, qaScore }) {
  const streamSummary = summarizeSse(streamResult.text);
  const usageSummary = usageConsistencySummary(streamSummary);
  const shapeSummary = responseShapeSummary(chatJson, model);
  const returnedModel = streamSummary.messageStartModel || shapeSummary.responseModel;
  const returnedModelMatches = Boolean(returnedModel && model && returnedModel === model);
  const qaDimensions = (qaScore.results || []).map((item) => item.dimensionZh || item.dimension);

  const modelIdentityStatus = classifyEvidence(
    returnedModel ? returnedModelMatches : null,
    false,
    "warning"
  );
  const protocolStatus = classifyEvidence(
    shapeSummary.looksCompatible,
    false,
    "warning"
  );
  const usageStatus = classifyEvidence(
    usageSummary.hasUsage ? usageSummary.consistent : null,
    false,
    "warning"
  );
  const streamStatus = classifyEvidence(
    Boolean(streamResult.ok && streamResult.stream && streamSummary.rawEventCount > 0),
    Boolean(streamResult.pending),
    "warning"
  );
  ...
  const items = [
    {
      ...
      labelZh: "流式证据",
      status: streamStatus,
      detail: evidenceStatusDetail(
        streamStatus,
        `Received ${streamSummary.rawEventCount} SSE events and normal stream chunks.`,
        "Stream chunks were incomplete or missing.",
        "No complete stream evidence was available in this fast run."
      ),
      detailZh: evidenceStatusDetail(
        streamStatus,
        `收到 ${streamSummary.rawEventCount} 个 SSE 事件，流式分片表现正常。`,
        "流式分片不完整，或没有拿到正常事件。",
        "这次快检没有拿到完整的流式证据。"
      ),
    },
    {
      ...
      detail: evidenceStatusDetail(
        protocolStatus,
        "Response includes normal choices content and looks OpenAI-compatible.",
        "Response shape drifted away from normal OpenAI-style fields.",
        "Not enough fields were returned to confirm the response shape."
      ),
      detailZh: evidenceStatusDetail(
        protocolStatus,
        "响应包含正常的 choices 内容，看起来符合 OpenAI 兼容结构。",
        "响应结构和常见 OpenAI 兼容字段存在偏差。",
        "返回字段不足，暂时无法确认响应结构。"
      ),
    },
    {
      key: "model_identity",
      label: "Returned model name",
      labelZh: "返回模型名",
      status: modelIdentityStatus,
      detail: evidenceStatusDetail(
        modelIdentityStatus,
        `Returned model matches the selected model: ${returnedModel}.`,
        `Returned model differs from the selected model: ${returnedModel || "unknown"}.`,
        "No returned model field was available for comparison."
      ),
      detailZh: evidenceStatusDetail(
        modelIdentityStatus,
        `返回模型名与所选模型一致：${returnedModel}。`,
        `返回模型名与所选模型不一致：${returnedModel || "未知"}。`,
        "返回中没有可用于比对的模型字段。"
      ),
    },
    {
      key: "usage_consistency",
      label: "Usage consistency",
      labelZh: "用量一致性",
      status: usageStatus,
      detail: evidenceStatusDetail(
        usageStatus,
        "Usage fields stayed internally consistent in this run.",
        "Usage fields looked inconsistent across the returned response.",
        "No stable usage fields were returned, so this item stays inconclusive."
      ),
      detailZh: evidenceStatusDetail(
        usageStatus,
        "本次返回里的 usage 字段前后基本一致。",
        "本次返回里的 usage 字段存在不一致迹象。",
        "没有返回稳定的 usage 字段，所以这一项暂时记为证据不足。"
      ),
    },
    {
      key: "qa_scope",
      ...
      labelZh: "题目覆盖",
      status: "pass",
      detail: `This fast run checks ${qaDimensions.join(", ")}.`,
      detailZh: `这次快检覆盖：${qaDimensions.join("、")}。`,
    },
  ];
...
function buildQualityChecks({ model, modelsJson, chatResult, chatJson, streamResult, qaScore }) {
  ...
  const protocolOk = chatResult.ok && responseOk;
...
function evidenceCompatibilityStatus(evidenceItems) {
  const failed = evidenceItems.filter((item) => item.status === "warning").length;
  const inconclusive = evidenceItems.filter((item) => item.status === "inconclusive").length;
  if (failed === 0 && inconclusive === 0) return "pass";
...
function latencyScore(latencyMs, qaScore) {
  ...
  let score = 0;
  if (chat <= 2500 && ttft <= 2500) score = 10;
  else if (chat <= 6000 && ttft <= 6000) score = 5;
...
async function runBatchQaCases({ chatUrl, apiKey, model, qaCases, timeoutMs = 12000 }) {
  const prompt = batchQaPrompt(qaCases);
  const source = "batch-stream";
  ...
  const answers = batchQaAnswers(answerText);
  if (!answers) throw new Error("Batch QA response was not parseable JSON.");
  ...
  const probeJson = {
    ...
    usage: streamUsageTokens(result.text),
  };
...
async function runQaCases({ chatUrl, apiKey, model, qaCases }) {
  try {
    return await runBatchQaCases({ chatUrl, apiKey, model, qaCases });
  } catch {
  ...
  for (let index = 0; index < qaCases.length; index += concurrency) {
    ...
    results.push(...(await Promise.all(batch.map((qaCase) => qaResultForCase({ chatUrl, apiKey, model, qaCase })))));
  }
  ...
  return {
    ...
    probeResult: {
      ...
      text: JSON.stringify(probeJson),
      timings: {
        firstByteMs: Math.max(...results.map((item) => item.latencyMs || 0)),
        totalMs: Math.max(...results.map((item) => item.latencyMs || 0)),
      },
    },
    probeJson,
    streamResult: {
...
function buildResponseBody({
  ...
  const evidence = detectionEvidence({ model, chatJson, streamResult, qaScore });
  const usage = usageTokens(chatJson);
  ...
  return {
    ...
    evidence: {
      status: evidenceCompatibilityStatus(evidence.items),
...
async function prepareCoreResults(payload, hostHeader) {
  ...
  const [modelsResult, qaBundle] = await Promise.all([
    timedFetch(
      modelsUrl,
      {
        method: "GET",
    ...
    runQaCases({ chatUrl, apiKey, model, qaCases: qaBank.cases }),
  ]);
  return {
    status: 200,
    baseUrl,
    ...
    streamResult: qaBundle.streamResult,
    chatUrl,
  };
}
